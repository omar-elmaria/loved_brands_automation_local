...
WITH original_asa AS (
    SELECT DISTINCT entity_id, country_code, asa_name, asa_id
...
additional_fields AS (
  SELECT 
    *,
    ...
    SUM(CASE WHEN unique_visits_pct_rank >= sessions_ntile_thr THEN num_orders ELSE NULL END) OVER (PARTITION BY entity_id, country_code, master_asa_id) AS asa_orders_after_visits_filter,
    SUM(
      CASE WHEN 
    ...
    COUNT(DISTINCT CASE WHEN unique_visits_pct_rank >= sessions_ntile_thr THEN vendor_code ELSE NULL END) OVER (PARTITION BY entity_id, country_code, master_asa_id) AS vendor_count_remaining_after_visits_filter,
...
ORDER BY region, entity_id, country_code, master_asa_id, cvr3_pct_rank DESC;
...
) b ON a.entity_id = b.entity_id AND a.country_code = b.country_code AND a.master_asa_id = b.master_asa_id AND a.vendor_code = b.vendor_code AND a.DF_total = b.min_df_total_of_vendor;
...
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_vendor_level_loved_brands_scaled_code` a 
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_vendor_level_loved_brands_scaled_code` b USING(entity_id, country_code, master_asa_id, vendor_code);
...
) b ON a.entity_id = b.entity_id AND a.country_code = b.country_code AND a.master_asa_id = b.master_asa_id AND a.DF_total = b.min_df_total_of_asa;
...
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_asa_level_loved_brands_scaled_code` a 
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_asa_level_loved_brands_scaled_code` b USING(entity_id, country_code, master_asa_id);
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN b.is_LB = 'Y' THEN b.vendor_code ELSE NULL END) OVER (PARTITION BY b.entity_id, b.country_code, b.master_asa_id) AS vendor_count_remaining_after_LB_logic,
    ...
    ROUND(COUNT(DISTINCT CASE WHEN b.is_LB = 'Y' THEN b.vendor_code ELSE NULL END) OVER (PARTITION BY b.entity_id, b.country_code, b.master_asa_id) / NULLIF(a.vendor_count_caught_by_asa, 0), 4) AS perc_vendors_remaining_after_LB_logic,
