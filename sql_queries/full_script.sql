...
INNER JOIN `dh-logistics-product-ops.pricing.active_entities_loved_brands_scaled_code` ent ON p.entity_id = ent.entity_id AND co.country_code = ent.country_code 
WHERE TRUE 
...
-- Step 3: Get the ASA setups in each country. This table displays the full setup of the ASA including conditions
...
WITH asa_setup_active_from_max_date_tmp_tbl AS (
    ...
    INNER JOIN `dh-logistics-product-ops.pricing.active_entities_loved_brands_scaled_code` ent ON asa.entity_id = ent.entity_id AND asa.country_code = ent.country_code -- Filter only for active DH entities
)
...
FROM (
    SELECT
        ...
        a.entity_id,
        a.country_code,
        -- Keep in mind that one scheme could be included in more than one ASA, so joining the on the "scheme_ids_per_asa_loved_brands_scaled_code" will produce duplicates and this is expected
        b.asa_id,
        b.asa_name,
...
WITH temp_tbl AS(
    ...
    LEFT JOIN `fulfillment-dwh-production.pandata_curated.pd_orders` pd ON o.entity_id = pd.global_entity_id AND o.platform_order_code = pd.code AND o.created_date = pd.created_date_utc
    INNER JOIN `dh-logistics-product-ops.pricing.active_entities_loved_brands_scaled_code` ent ON o.entity_id = ent.entity_id AND o.country_code = ent.country_code -- Filter only for active DH entities
    WHERE TRUE
...
WITH all_metrics AS ( 
    SELECT 
        ...
        COALESCE(COUNT(DISTINCT CASE WHEN event_action = 'shop_details.loaded' THEN events_ga_session_id ELSE NULL END), 0) AS num_unique_vendor_visits, -- If a vendor was visited more than once in the same session, it's one visit
        COALESCE(COUNT(DISTINCT CASE WHEN event_action = 'shop_details.loaded' THEN event_time ELSE NULL END), 0) AS num_total_vendor_impressions, -- If a vendor was visited more than once in the same session, count all impressions
        ...
        COALESCE(
          ROUND(COUNT(DISTINCT CASE WHEN event_action = 'transaction' THEN event_time ELSE NULL END) / NULLIF(COUNT(DISTINCT CASE WHEN event_action = 'shop_details.loaded' THEN events_ga_session_id ELSE NULL END), 0), 5)
          , 0
        ) AS cvr3,
    FROM `dh-logistics-product-ops.pricing.vendor_ids_per_asa_loved_brands_scaled_code` v
    LEFT JOIN `dh-logistics-product-ops.pricing.session_data_for_vendor_screening_loved_brands_scaled_code` s 
        ON TRUE 
    ...
    LEFT JOIN `dh-logistics-product-ops.pricing.order_data_for_vendor_screening_loved_brands_scaled_code` o 
        ON TRUE 
...
pct_ranks AS (
    SELECT
        *,
        ROUND(PERCENT_RANK() OVER (PARTITION BY entity_id, country_code, asa_id ORDER BY num_orders), 4) AS orders_pct_rank,
        ROUND(PERCENT_RANK() OVER (PARTITION BY entity_id, country_code, asa_id ORDER BY num_unique_vendor_visits), 4) AS unique_visits_pct_rank,
        ROUND(PERCENT_RANK() OVER (PARTITION BY entity_id, country_code, asa_id ORDER BY cvr3), 4) AS cvr3_pct_rank
    FROM all_metrics
),
...
additional_fields AS (
  SELECT 
    *,
    ...
    SUM(CASE WHEN unique_visits_pct_rank >= sessions_ntile_thr THEN num_orders ELSE NULL END) OVER (PARTITION BY entity_id, country_code, asa_id) AS asa_orders_after_visits_filter,
    SUM(
      CASE WHEN 
        unique_visits_pct_rank >= sessions_ntile_thr 
        AND orders_pct_rank >= orders_ntile_thr 
      THEN num_orders ELSE NULL END
    ...
    COUNT(DISTINCT CASE WHEN unique_visits_pct_rank >= sessions_ntile_thr THEN vendor_code ELSE NULL END) OVER (PARTITION BY entity_id, country_code, asa_id) AS vendor_count_remaining_after_visits_filter,
    
    COUNT(
      DISTINCT CASE WHEN 
        unique_visits_pct_rank >= sessions_ntile_thr 
        AND orders_pct_rank >= orders_ntile_thr 
      THEN vendor_code ELSE NULL END
...
ORDER BY region, entity_id, country_code, asa_id, cvr3_pct_rank DESC;
...
WITH dps_logs_stg_1 AS (
    ...
    INNER JOIN `dh-logistics-product-ops.pricing.city_data_loved_brands_scaled_code` cd ON logs.entity_id = cd.entity_id AND ST_CONTAINS(cd.zone_shape, logs.customer.location) -- Filter for sessions in the zones specified above
    WHERE TRUE
...
FROM `dh-logistics-product-ops.pricing.session_data_for_vendor_screening_loved_brands_scaled_code` x
-- With this join, ~ 12.9% of all rows get a NULL DF_total as a result of filtering out multipleFee endpoint requests
LEFT JOIN `dh-logistics-product-ops.pricing.dps_logs_loved_brands_scaled_code` logs -- You can use an INNER JOIN here if it's important to have a DF value associated with every session
    ON TRUE
...
WHERE TRUE 
    ...
    AND CONCAT(ven.entity_id, ' | ', ven.country_code, ' | ', ven.asa_id, ' | ', ses.DF_total) IN (
        SELECT DISTINCT CONCAT(entity_id, ' | ', country_code, ' | ', asa_id, ' | ', fee) 
...
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_vendor_level_loved_brands_scaled_code` a 
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_vendor_level_loved_brands_scaled_code` b USING(entity_id, country_code, asa_id, vendor_code);
...
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_asa_level_loved_brands_scaled_code` a 
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_asa_level_loved_brands_scaled_code` b USING(entity_id, country_code, asa_id);
...
SELECT
    ...
    SUM(CASE WHEN a.pct_chng_of_actual_cvr3_from_base > c.pct_chng_of_asa_cvr3_from_base THEN 1 ELSE 0 END) OVER (PARTITION BY a.entity_id, a.country_code, a.asa_id, a.vendor_code) AS num_yes
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_vendor_level_loved_brands_scaled_code` a
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_asa_level_loved_brands_scaled_code` b
    ON TRUE
        AND a.entity_id = b.entity_id
        AND a.country_code = b.country_code
        AND a.asa_id = b.asa_id
LEFT JOIN `dh-logistics-product-ops.pricing.cvr_per_df_bucket_asa_level_loved_brands_scaled_code` c
    ON TRUE
        AND a.entity_id = c.entity_id 
        AND a.country_code = c.country_code
        AND a.asa_id = c.asa_id 
