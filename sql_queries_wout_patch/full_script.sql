...
ORDER BY region ASC, entity_id ASC, country_code ASC, asa_id ASC, cvr3_pct_rank DESC
;
...
WHERE TRUE
    ...
    AND CONCAT(ven.entity_id, " | ", ven.country_code, " | ", ven.asa_id, " | ", ses.df_total) IN (
        SELECT DISTINCT CONCAT(entity_id, " | ", country_code, " | ", asa_id, " | ", fee) AS entity_country_asa_fee
...
INNER JOIN b ON a.entity_id = b.entity_id AND a.country_code = b.country_code AND a.asa_id = b.asa_id AND a.vendor_code = b.vendor_code AND a.df_total = b.min_df_total_of_vendor
;
...
FROM `dh-logistics-product-ops.pricing.cvr_per_df_bucket_vendor_level_loved_brands_scaled_code` AS a
LEFT JOIN `dh-logistics-product-ops.pricing.df_and_cvr3_at_min_tier_vendor_level_loved_brands_scaled_code` AS b USING (entity_id, country_code, asa_id, vendor_code)
;
...
INNER JOIN b ON a.entity_id = b.entity_id AND a.country_code = b.country_code AND a.asa_id = b.asa_id AND a.df_total = b.min_df_total_of_asa
;
